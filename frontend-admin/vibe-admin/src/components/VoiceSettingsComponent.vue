<template>
  <v-card class="pa-6 mx-auto" elevation="10" rounded="xl">
    ...
    <v-card-text class="d-flex flex-column justify-center" style="height: calc(100% - 80px);">
      <div class="text-body-1 mb-4">Schakel voice functionaliteit in of uit </div>

      <div class="toggle-container">
        <button class="toggle-button" :class="{ active: isActive }" @click="toggleActive">
          {{ isActive ? "Voice functionaliteit: Aan" : "Voice functionaliteit: Uit" }}
        </button>
      </div>
    </v-card-text>
  </v-card>
...
async function fetchRouteEnabled() {
  try {
    const response = await axios.get(`${API_BASE_URL}admin/api/settings/voice-settings`);
    isActive.value = response.data;
  } catch (error) {
    console.error('Error fetching voice status:', error);
    alert('Fout bij ophalen van voice status.');
  }
}

async function toggleActive() {
  try {
    await axios.post(`${API_BASE_URL}admin/api/settings/voice-settings`, {
      enabled: !isActive.value
    });
    isActive.value = !isActive.value; // Toggle the button state
  } catch (error) {
    console.error('Error toggling voice status:', error);
    alert('Fout bij bijwerken van voice status.');
  }
}
